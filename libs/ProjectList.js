...
// 合约部署实际上是发起了 1 笔交易，交易的接收者为空，以太坊将接收者为空的交易默认为是合约实例创建请求, 这类交易中会携带当前部署合约的机器码。部署成功的话会返回新建的合约账户，合约部署本身需要消耗 Gas，这个费用是发起者支付的
...
const contract = new web3.eth.Contract(JSON.parse(ProjectList.inteface), address)
